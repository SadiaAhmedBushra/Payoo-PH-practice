...
function handleToggleActiveButton(activeButtonId) {
  ...
  document
    ...
    .classList.add("border-blue-500", "bg-[#0874f20d]");
}

// Add money button event listener

document
  .getElementById("add-amount-button")
  .addEventListener("click", function (e) {
    ...
    if (userAccountNumber.length !== 11) {
      alert(
        "Account number must be 11 digits long. Please try again with a valid account number."
      );
      return;
    }
...
document
  .getElementById("withdraw-amount-button")
  .addEventListener("click", function (e) {
    e.preventDefault();
    ...
    if (userAgentNumber.length !== 11) {
      alert(
        "Agent number must be 11 digits long. Please try again with a valid agent number."
      );
      return;
    }
    ...
    const availableBalance = getInnerTextNumber("available-balance");
    if (userAmountToWithdraw > availableBalance) {
...
document
  ...
  .addEventListener("click", function (e) {
    ...
    if (userAmountToTransfer > availableBalance) {
      alert("Sorry! You do not have enough amount.");
      return;
    }
    // console.log(availableBalance);
...
// Get Bonus button event listener

document
  .getElementById("get-now-button")
  .addEventListener("click", function (e) {
    e.preventDefault();
...
document
  ...
  .addEventListener("click", function (e) {
    ...
    const availableBalance = getInnerTextNumber("available-balance");
    if (userAmountToPay > availableBalance) {
      alert("Sorry! You do not have enough amount.");
      return;
    }
